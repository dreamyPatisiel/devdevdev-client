    ...
    <div className='relative'>
      ...
      <span className='text-c1 font-bold text-gray5'>
        <label htmlFor='myvote-check' className='inline-flex items-center cursor-pointer'>
          ...
          />
          <span
            className={`text-gray4 c1 font-bold flex mr-3 ${isChecked ? 'text-secondary300' : ''} ${loginStatus === 'logout' ? 'cursor-not-allowed' : ''}`}
          >
          ...
          >
            <div
              className={cn(
                `absolute top-[3.75px] right-[0.4rem] rounded-full h-5 w-5 transition-all ${isChecked ? 'bg-secondary500 translate-x-[-1.5rem]' : 'bg-gray4'}`,
              )}
            ></div>
          </div>
        </label>
